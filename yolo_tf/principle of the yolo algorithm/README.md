...
>* 另外两个小细节：1、作者先在ImageNet数据集上预训练网络，而且网络只采用fig3的前面20个卷积层，输入是224x224大小的图像。然后在检测的时候再加上随机初始化的4个卷积层和2个全连接层，同时输入改为更高分辨率的448X448。2、Relu层改为leakyRelu，即当x<0时，激活值是0.1xX，而不是传统的0。
#
6、损失函数
-------
...
[1] 位置相关误差（坐标、IOU）与分类误差对网络loss的贡献值是不同的，因此YOLO在计算loss时，使用\lambda _{coord} =5修正coordError。
